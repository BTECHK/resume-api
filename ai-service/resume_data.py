"""
Structured resume content for RAG ingestion.
This is the single source of truth for the candidate's resume data,
used by both the email bot and chatbot.
"""
...
RESUME_SECTIONS = {
    ...
    "summary": (
        ...
        "delivering end-to-end solutions — from prototypes and demos to production "
        "systems serving 50,000+ users — while conducting business analysis, building "
        "analytics dashboards, and advising executive stakeholders on strategic tradeoffs. "
        "Expert at technical troubleshooting across cloud infrastructure, data platforms, "
        "and enterprise systems. Skilled at identifying business opportunities through "
        "technical solutions that drive measurable outcomes: 30% cost reduction, 40% "
        "security risk reduction, and 100% regulatory compliance."
    ),
    "skills": {
        ...
        "cloud_infrastructure": [
            ...
            "GCP (Cloud Run, BigQuery, Compute Engine, Artifact Registry, Vertex AI)",
            "OCI (Compute, VCN, Object Storage, OCIR)",
    ...
    "certifications": [
        ...
        "CompTIA Security+ ce",
        "Oracle Financials Cloud ERP Certification",
    ],
    "portfolio_projects": [
        {
            "name": "Resume API (GCP)",
            "description": "Production-grade REST API serving resume data with analytics pipeline. FastAPI, SQLite, BigQuery, Cloud Run, Docker, n8n email bot, Gemini RAG chatbot. Demonstrates API design, SQL optimization, data engineering, and AI integration on GCP free tier.",
        },
        {
            "name": "OCI Federal Lab",
            "description": "3-phase Oracle Cloud Infrastructure lab simulating a federal landing zone. Terraform IaC, Ansible automation, bastion architecture, FedRAMP compliance agent. Demonstrates enterprise cloud deployment patterns.",
        },
        {
            "name": "WoodBuilds.lol",
            "description": "League of Legends companion app. React frontend with champion build recommendations. Demonstrates full-stack React development with gaming API integration.",
        },
        {
            "name": "Liniage Link",
            "description": "React/Express family tree application for the African diaspora. Full-stack JavaScript with data visualization for genealogical connections.",
        },
    ],
}
...
def get_resume_as_text() -> str:
    """Flatten resume into plain text chunks suitable for embedding."""
    sections = []
    ...
    # Skills
    skills_text = "# Skills\n"
    for category, skill_list in RESUME_SECTIONS["skills"].items():
    ...
    # Experience
    for job in RESUME_SECTIONS["experience"]:
    ...
    # Education
    edu = RESUME_SECTIONS["education"][0]
    sections.append(
    ...
    cert_text += "\n".join(f"- {cert}" for cert in RESUME_SECTIONS["certifications"])
    sections.append(cert_text)

    # Portfolio
    portfolio_text = "# Portfolio Projects\n"
    for project in RESUME_SECTIONS["portfolio_projects"]:
        portfolio_text += f"\n## {project['name']}\n{project['description']}\n"
