...
**Live Demo:** `https://resume-api-xxxxx-uc.a.run.app` *(update after deployment)*  
**Author:** [Your Name]
...
- [Scale Benchmarks: 10K → 500K → 5M Rows](#scale-benchmarks-10k--500k--5m-rows)
...
### Example Response: `GET /resume/skills?category=databases`

```json
{
  "category": "databases",
  "skills": [
    {"name": "SQL", "proficiency": "project_experience", "platforms": ["Oracle RDBMS", "SQL Server", "Postgres"]},
    {"name": "PL/SQL", "proficiency": "training"},
    {"name": "BigQuery", "proficiency": "project_experience"}
  ],
  "metadata": {
    "total_skills_in_category": 3,
...
{
  "top_domains": [
    {"domain": "google.com", "hits": 1842, "avg_response_ms": 45.2},
    {"domain": "amazon.com", "hits": 1567, "avg_response_ms": 52.1},
    {"domain": "deloitte.com", "hits": 1203, "avg_response_ms": 38.7}
  ],
  "method": "dictionary_frequency_map",
  "complexity": "O(n) scan + O(k log k) sort for top-k",
  "total_unique_domains": 10
}
```
...
> This section demonstrates how query optimization changes as data volume grows — the core skill needed when advising clients on BigQuery best practices.
...
> Same task — "find the most frequently appearing domain" — run with different tools at different data volumes. This demonstrates when tool selection matters and how to make defensible engineering decisions about data processing approaches.

### The Experiment

Each benchmark script answers the same question using multiple approaches, then measures time, memory, and (where applicable) bytes scanned:

| Scale | Script | Approaches Compared | Key Observation |
|-------|--------|---------------------|-----------------|
| 10K rows | `benchmark_small.py` | Python dict, Counter, SQLite GROUP BY | All instant — tool choice is preference, not performance |
| 500K rows | `benchmark_medium.py` | Python dict, Counter, SQLite, BigQuery naive, BigQuery CTE+window | Crossover zone — Python still works but measurably slower. BigQuery overhead visible but analytical power increases |
| 5M rows | `benchmark_large.py` | Python dict (streaming from BQ), BigQuery naive, BigQuery partitioned | Python chokes on streaming 5M rows. BigQuery handles it in seconds. Partitioning reduces bytes scanned dramatically |

### How the 5M Row Table Was Generated

Instead of generating and uploading more CSV data, the 5M row table was created inside BigQuery using a `CROSS JOIN`:

```sql
CREATE TABLE `resume_analytics.recruiter_queries_5m` AS
SELECT ROW_NUMBER() OVER() AS query_id, a.*
FROM `resume_analytics.recruiter_queries` a          -- 500K rows
CROSS JOIN UNNEST(GENERATE_ARRAY(1, 10)) AS multiplier;  -- × 10 = 5M
```

This demonstrates SQL-native data generation — no ETL pipeline, no file uploads, no Python scripts. The data stays inside BigQuery.

### Results Summary

*Screenshots from actual benchmark runs:*

**10K rows** — all approaches < 0.01s, negligible memory:

![Benchmark: 10K rows](docs/screenshots/step-3.5-benchmark-small-10k.png)

**500K rows** — the crossover point:

![Benchmark: 500K rows](docs/screenshots/step-3.6-benchmark-medium-500k.png)

**5M rows** — generating 5M rows via CROSS JOIN, then benchmarking:
